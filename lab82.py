...
def gameLoop():
    ...
    foodx = round(random.randrange(0, WIDTH - sn-10) / 10.0) * 10.0
    foody = round(random.randrange(0, HEIGHT - sn-10) / 10.0) * 10.0
    ...
    while not game_over:
        ...
        y1 += y1_change
        SCREEN.fill(WHITE)
        pygame.draw.rect(SCREEN, GREEN, [foodx, foody, sn, sn])
        snake_Head = []
        ...
        if x1 == foodx and y1 == foody:
            foodx = round(random.randrange(0, WIDTH - sn - 50) / 10.0) * 10.0
            foody = round(random.randrange(0, HEIGHT - sn - 50) / 10.0) * 10.0
            length += 1
